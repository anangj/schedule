<x-app-layout>
    <div class="space-y-8">
        <div class="block sm:flex items-center justify-between mb-6">
            ...
            <div class="text-end">
                <a class="btn inline-flex justify-center btn-dark rounded-[25px] items-center !p-2 !px-3" href="{{ route('master-nod.index') }}">
    ...
    <div class="rounded-md overflow-hidden">
        <div class="bg-white dark:bg-slate-800 px-5 py-7">
            <form method="POST" action="{{ route('master-nod.update', $masterNod) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="form-group">
                    <label for="employee_name">Nama Nod</label>
                    <input type="text" id="employee_name" name="employee_name" class="form-control" value="{{ $masterNod->employee_name }}">
                </div>
                <div class="form-group">
                    <label for="employee_id">ID Karyawan</label>
                    <input type="text" id="employee_id" name="employee_id" class="form-control" value="{{ $masterNod->employee_id }}">
                </div>
                <div class="form-group">
                    ...
                    <img id="image_preview" src="{{ $masterNod->image_url ? Storage::url($masterNod->image_url) : '#' }}" alt="Preview Foto Dokter" class="block w-32 h-32 object-cover rounded-full mt-2">
                </div>
