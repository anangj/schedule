            ...
            <div class="md:flex justify-between items-center mb-6">
                ...
                @if (Auth::user()->hasRole('super-admin'))
                    <div class="flex flex-wrap ">
                        <form action="{{ route('master-nod.uploadExcel') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                                                ...
                                                <td class="table-td ">
                                                    <div class="flex space-x-3 rtl:space-x-reverse">
                                                        <button class="action-btn" type="button">
                                                            <a href="{{ route('master-nod.show', $item->id) }}"
                                                                class="action-btn">
                                                                <iconify-icon icon="heroicons:eye"></iconify-icon>
                                                            </a>
                                                        </button>
                                                        <button class="action-btn" type="button">
                                                            <a href="{{ route('master-nod.edit', $item->id) }}"><iconify-icon
                                                                    icon="heroicons:pencil-square"></iconify-icon></a>
                                                        </button>
                                                        <form id="deleteForm{{ $item->id }}" method="POST"
                                                            action="{{ route('master-nod.destroy', $item->id) }}"
