    ...
    <div class="space-y-8">
        <div class="block sm:flex items-center justify-between mb-6">
            ...
            <div class="flex justify-end">
                <a href="{{ route('ponek.downloadTemplate') }}" class="btn inline-flex justify-center btn-dark dark:bg-slate-700 dark:text-slate-300 m-1">
                    <span class="flex items-center">
                        <span>Template</span>
                    </span>
                </a>
                <form  class="form-control" action="{{ route('ponek.uploadExcel') }}" method="POST"
        ...
        <!-- Filter Notification -->
        @if (request()->filled('employee_name') || request()->filled('date') || request()->filled('name_shift'))
            <div class="mb-4 p-4 bg-yellow-100 text-yellow-800 rounded-md">
                <p><strong>Notice:</strong> The results are currently filtered.</p>
                <a href="{{ route('ponek.index') }}" class="text-blue-500 underline">Clear Filters</a>
            </div>
        @endif
            ...
            <div class="card">
                <div class="card-header noborder -mb-6">
                    ...
                    <div id="filterForm" class="card mb-6 p-5 hidden">
                        <form method="POST" action="{{ route('ponek.index') }}">
                            @csrf
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <!-- Filter by Nurse Name -->
                                <div>
                                    <label for="employee_name">Ponek Name</label>
                                    <select name="employee_name" id="employee_name" class="form-control">
                                        <option value="">Select a Ponek</option>
                                        @foreach ($listPonek as $item)
                            ...
                            <div class="mt-4">
                                ...
                                <a href="{{ route('ponek.index') }}" class="btn btn-light">Reset</a>
                            </div>
                        </form>
                    </div>
                </div>
                                                ...
                                                <td class="table-td ">
                                                    <div class="flex space-x-3 rtl:space-x-reverse">
                                                        <button class="action-btn" type="button">
                                                            <a href="{{ route('ponek.show', $item->id) }}"
                                                                class="action-btn">
                                                                <iconify-icon icon="heroicons:eye"></iconify-icon>
                                                            </a>
                                                        </button>
                                                        <button class="action-btn" type="button">
                                                            <a href="{{ route('ponek.edit', $item->id) }}"><iconify-icon
                                                                    icon="heroicons:pencil-square"></iconify-icon></a>
                                                        </button>
                                                        <form id="deleteForm{{ $item->id }}" method="POST"
                                                            action="{{ route('ponek.destroy', $item->id) }}"
