    ...
    <div class="space-y-8">
        <div class="block sm:flex items-center justify-between mb-6">
            ...
            <div class="flex justify-end">
                <a href="{{ route('nod.downloadTemplate') }}" class="btn inline-flex justify-center btn-dark dark:bg-slate-700 dark:text-slate-300 m-1">
                    <span class="flex items-center">
                        <span>Template</span>
                    </span>
                </a>
                <form  class="form-control" action="{{ route('nod.uploadExcel') }}" method="POST"
        ...
        @if (request()->filled('employee_name') || request()->filled('date') || request()->filled('name_shift'))
            <div class="mb-4 p-4 bg-yellow-100 text-yellow-800 rounded-md">
                ...
                <a href="{{ route('nod.index') }}" class="text-blue-500 underline">Clear Filters</a>
            </div>
        @endif
                    ...
                    <div id="filterForm" class="card mb-6 p-5 hidden">
                        <form method="POST" action="{{ route('nod.index') }}">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <div>
                                    <label for="employee_name">Nod Name</label>
                                    <select name="employee_name" id="employee_name" class="form-control">
                                        <option value="">Select a Nod</option>
                                        @foreach ($listNod as $item)
                                                ...
                                                <td class="table-td ">
                                                    <div class="flex space-x-3 rtl:space-x-reverse">
                                                        <button class="action-btn" type="button">
                                                            <a href="{{ route('nod.show', $item->id) }}"
                                                                class="action-btn">
                                                                <iconify-icon icon="heroicons:eye"></iconify-icon>
                                                            </a>
                                                        </button>
                                                        <button class="action-btn" type="button">
                                                            <a href="{{ route('nod.edit', $item->id) }}"><iconify-icon
                                                                    icon="heroicons:pencil-square"></iconify-icon></a>
                                                        </button>
                                                        <form id="deleteForm{{ $item->id }}" method="POST"
                                                            action="{{ route('nod.destroy', $item->id) }}"
